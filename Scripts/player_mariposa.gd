...
enum CorMariposa { NENHUMA, BRANCA, PRETA }
...
func definir_cor(cor_escolhida: CorMariposa) -> void:
	...
	match cor_atual:
		...
		CorMariposa.PRETA:
			anim.modulate = Color(0.14, 0.14, 0.16, 1)
	pode_mover = true
...
func _physics_process(delta: float) -> void:
	...
	if (Input.is_action_pressed("ui_select") or Input.is_action_pressed("interacao")) and not zonas_proximas.is_empty():
		if not agarrado:
			iniciar_agarro()
	...
	move_and_slide()
	global_position.x = clampf(global_position.x, 12.0, 628.0)
	global_position.y = clampf(global_position.y, 58.0, 308.0)
...
func iniciar_agarro() -> void:
	zona_atual = encontrar_zona_mais_proxima()
	if not zona_atual:
		return
	agarrado = true
	anim.stop()
...
func _on_area_detector_entered(area: Area2D) -> void:
	if area.is_in_group("zonas_camuflagem") and not zonas_proximas.has(area):
		zonas_proximas.append(area)

func _on_area_detector_exited(area: Area2D) -> void:
	if zonas_proximas.has(area):
		zonas_proximas.erase(area)
	if area == zona_atual:
		if agarrado:
			soltar_agarro()

func encontrar_zona_mais_proxima() -> Area2D:
	var melhor_zona: Area2D = null
	var melhor_distancia := INF
	for zona in zonas_proximas:
		if not is_instance_valid(zona):
			continue
		var distancia := global_position.distance_squared_to(zona.global_position)
